...
def series_marks(text: str) -> frozenset[str]:
    ...
    ① block.py の辺3（編集距離によるタイポ検出）の枝刈り（`ma == marks[b]`）。
    「Aiobahn」と「Aiobarn」のように印が一致する組だけをタイポ候補にする
    ために使っていて、ここでの「印が食い違う＝別物」という判断はこれからも
    変わらない（外すと artist 側で誤った辺が増えるので、この条件自体は
    今後も残す）。

    ② block.py のヒント生成（series-mark-mismatch）。「響け!ユーフォニアム」と
    「響け!ユーフォニアム3」、「てーきゅう 2期」と「てーきゅう 4期」のように、
    印が食い違う値が同じクラスタに混ざっていることを知らせるためのフラグ。
    **work ではもう「統合してはいけない」根拠ではない**（同じブランドのシーズン
    違いは積極的に統合する方針になったため）。それでも「どの表記がどのシーズンを
    指すか」を人間が把握する手掛かりとしては引き続き有用なので、フラグ自体は
    出し続ける。artist では合同名義などの手掛かりとして従来の意味のまま。
    """
