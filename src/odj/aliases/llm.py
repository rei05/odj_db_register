...
逆算していて、入力 4,200 + 出力 3,700 = 7,900 が1リクエストの上限
（TPM_LIMIT の説明を参照）。
実測（--dry-run）で work の 151 クラスタが 22 リクエスト、
artist の 105 クラスタが 23 リクエスト（work は「ブランド単位でまとめる」方針転換で
...
# 最大 15 クラスタしか詰まらないので必要量は 3,000tok。**25% の余裕を足してある**
...
MAX_OUTPUT_TOKENS = 3700

# 入力に使える残り。TPM から出力ぶんを引くと 4,300 だが、100 だけ余らせてある。
# estimate_tokens は実測より1〜2割多めに出るので、この値自体は安全側に外れる。
#
# **入力と出力はトレードオフ**で、入力を増やすと1回に詰まるクラスタが増え、
# 必要な出力も増える。合計が TPM を超えない範囲で一番リクエスト数が減るのが
# この配分（入力 4,200 + 出力 3,700 = 7,900）。動かすときは必ず両方を見ること。
# **動かすとプロンプト全文の SHA256 が変わってキャッシュが全部無効になる。**
SAFE_INPUT_TOKENS = 4200

# 1リクエストに詰めるクラスタ数の上限。**出力枠から逆算した値**で、これを超えると
# 出力が足りずに答えが切れる。実際には入力側が先に埋まって 15 クラスタ前後で
