...
from odj.aliases import auto, block, llm, sources, store
...
# ---------------------------------------------------------------------------
# auto（規則に合う提案だけを人手を通さず承認する。書くのは *.auto.toml だけ）
# ---------------------------------------------------------------------------

# 1件あたりに並べる表記の数。全部出すと1行が長くなりすぎるクラスタがある
# （「アイカツ!」は 9 表記）。
_SHOW_VARIANTS = 4


def _join(values: list[str], limit: int = _SHOW_VARIANTS) -> str:
    head = "、".join(values[:limit])
    return head + (f" ほか{len(values) - limit}件" if len(values) > limit else "")


def _print_auto(result: dict) -> None:
    """--dry-run の中身。**何件がなぜ人間に残るか**が分かる形にする。

    ask の _print_plan と同じ趣旨で、「実行前に結果を全部見せる」ためのもの。
    自動承認は取り消せる（--undo）とはいえ、辞書に approved = true を書く操作
    なので、投げる前に中身を確かめられないと使い物にならない。
    """
    field = result["field"]
    accepted, skipped = result["accepted"], result["skipped"]
    values = sum(len(a["variants"]) for a in accepted)
    rows = sum(a["rows"] for a in accepted)
    # 2回目以降の実行では、前回承認したぶんが auto-done として skipped に入る。
    # これを「人間に残す」に数えると、1度回したあとの表示が
    # 「自動承認 0 個 / 人間に残す 150 個」になって、何も片付いていないように
    # 見えてしまう。**既に自動承認済みのぶんは分けて数える。**
    done = [s for s in skipped if s["code"] == "auto-done"]
    remaining = [s for s in skipped if s["code"] != "auto-done"]
    head = f"[{field}] クラスタ {result['total']} 個 → 自動承認 {len(accepted)} 個"
    if done:
        head += f"（既に承認済み {len(done)} 個）"
    print(head + f" / 人間に残す {len(remaining)} 個")
    print(f"  自動承認するぶん: {values} 表記 / {rows} 行")
    if remaining:
        counts: dict[str, list[dict]] = {}
        for item in remaining:
            counts.setdefault(item["code"], []).append(item)
        print("  人間に残す理由の内訳:")
        for code, label in auto.SKIP_LABELS.items():
            hits = counts.get(code)
            if not hits:
                continue
            print(f"    {label}: {len(hits)} 個")
            # 例を1件だけ出す。内訳の数字だけでは、規則のどれが効きすぎて
            # いるのかを確かめられない。
            example = hits[0]
            detail = f" … {example['detail']}" if example["detail"] else ""
            print(f"      例) {example['id']} {_join(example['values'])}{detail}")
    if accepted:
        print("  自動承認する正準名:")
        for item in accepted:
            print(f"    {item['canonical']}  ← {len(item['variants'])} 表記: "
                  f"{_join(item['variants'])}")


def _cmd_auto(args: argparse.Namespace) -> int:
    try:
        if args.undo:
            result = auto.undo(args.field, dry_run=args.dry_run)
            removed = result["removed"]
            print(f"[{args.field}] 自動承認したぶん {len(removed)} 件を取り消し"
                  + ("ます" if args.dry_run else "ました"))
            for item in removed:
                print(f"    {item['canonical']}  ← {_join(item['variants'])}")
            if removed and not args.dry_run:
                print("  これらの値は再び候補（未判断）に戻ります。"
                      "odj.aliases block をもう一度回すとレビューのキューに出ます")
        else:
            result = auto.run(args.field, dry_run=args.dry_run)
            _print_auto(result)
    except AliasError as exc:
        print(f"失敗: {exc}", file=sys.stderr)
        return 1
    if args.dry_run:
        print(f"[{args.field}] --dry-run のため、ファイルは1つも書き変えていません")
    elif result["wrote"]:
        print("  書いた: " + " / ".join(result["wrote"]))
    return 0
...
def _auto_where(field: str, cluster_id: str) -> str:
    ...
    # 「第2回 せーや ほか」「第2回〜第10回 tri ほか」の形。組み立ては block 側
    # （自動承認も同じ形の where を書くので、クラスタ JSON を知っている側に置いた）。
    return block.where_hint(cluster) if cluster else ""
...
def _accept(payload: dict, field: str, cluster_id: str, reason: str, where: str) -> list[str]:
    ...
    # ここから3つの検査は store 側に置いて**自動承認（auto.py）と共有**している。
    # 人間の判断と自動承認で通る検査が違うと、緩いほうの経路から壊れた同値クラスが
    # 辞書に入る。
    store.check_canonical(field, cluster_id, canonical, variants)
    values = variants + ([canonical] if canonical not in variants else [])
    store.check_keep_apart(values, block.load_keep_apart())
    store.check_conflict(values, canonical, store.variant_index(store.load_entries(field)))
...
def decide(payload: dict) -> dict[str, Any]:
    ...
    if action in ("accept", "reject"):
        store.check_already_decided(
            _string_list(payload, "variants"), store.decided_index(field)
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_auto = sub.add_parser(
        "auto",
        help="規則に合う提案だけを自動承認して <field>s.auto.toml に書く"
             "（ネットワーク不要。人が育てる辞書には触らない）",
    )
    p_auto.add_argument(
        "--field", choices=list(block.FIELD_KEYS), default="work",
        help="対象フィールド（既定: work）",
    ...
    p_auto.add_argument(
        "--dry-run", action="store_true",
        help="何も書かず、自動承認する正準名と、人間に残るクラスタの理由の内訳だけ出す",
    )
    p_auto.add_argument(
        "--undo", action="store_true",
        help="自動承認したぶんを全部取り消す（*.auto.toml を空にする）。"
             "取り消した値は再び候補に戻る",
    )
